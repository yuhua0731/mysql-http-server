...
def main():
    ...
    parser.add_argument(
        "config_array", help="file name which contains bytearray to get configurations", type=str
    )
    ...
    for x in range(test):
        with open(config, "r") as csvfile:
            ...
            for config_row in config_plots:
                get_config_msg = bytearray.fromhex(config_row[0])
                test_mqtt.config_reply = bytearray(b"\x43") + get_config_msg[1:4]
                test_mqtt.flag_config = True
                res = test_mqtt.request_config(client, get_config_msg)
                if res[0] == "pass":
                    pass_count += 1
                if res[0] == "timeout":
                    timeout_count += 1
